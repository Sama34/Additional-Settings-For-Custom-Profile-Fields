...
$l['xt_proffields_cinp_desc'] = 'We can place the input field anywhere in our edit profile page template (usercp_profile) and/or member_register template. To do this, at least put <em>{INPUT}</em> element in this setting, then put <em>{$GLOBALS[\'xtpf_inp\'][\'fidX\']}</em> variable anywhere in the edit profile template (usercp_profile) and/or member_register template, where X is the profile field id. Please remember that when we put something to this setting, then the input fields won\'t be displayed in the default Custom Profile Fields inputs block. Leave it blank if we want to use the default input.';
...
$l['xt_proffields_df_desc'] = '<strong>Use <em>{VALUE}</em> for representating the value of this field.</strong> We can use conditional structures in this setting. We need to put <em>{$xtpf[\'fidX\']}</em> or <em>{$GLOBALS[\'xtpf\'][\'fidX\']}</em> manually in our template for displaying it, where X is this field\'s ID.';
$l['xt_proffields_mvd'] = 'Multiple Value Delimiter (Checkboxes)';
$l['xt_proffields_mvd_desc'] = 'The delimiter used to separate multiple values when displayed. This value is not parsed (i.e. you can use HTML etc here).';
